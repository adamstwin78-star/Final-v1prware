...
win:EditOpenButton({
    ...
    StrokeThickness = 0,
    Color           = ColorSequence.new(Color3.fromHex("#7c3aed"), Color3.fromHex("#4f46e5")),
    OnlyMobile      = true,
    Enabled         = true,
    Draggable       = true,
})
...
secGenAuto:Toggle({ Title = "Auto Solve", Type = "Checkbox", Flag = "flowOn", Default = flow.on, Callback = function(on) flow.on = on; cfg.set("flowOn", on) end })
...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: VISUAL (ESP) — event-driven rewrite, zero polling
------------------------------------------------------------------------
------------------------------------------------------------------------
...
--[[
    REWRITE NOTES — why the old code caused micro-freezes:
    1. A task.spawn loop ran every 3 seconds calling espDoKillers/Survivors/etc.,
       which iterated ALL workspace descendants inside task.spawn threads — causing
       GC spikes and thread churn every 3 seconds.
    2. espDoItems called svc.WS:GetDescendants() (potentially thousands of objects)
       on every refresh cycle.
    3. espAttach used task.wait(0.2) inside event callbacks (ChildAdded), stalling
       those threads and backing up the scheduler.
    4. Multiple concurrent task.spawn inside refresh loop competed for the scheduler.

    FIX:
    - ESP is now 100% event-driven. ChildAdded/ChildRemoved handle all live updates.
    - No refresh loop. No task.spawn churn. No GetDescendants on world.
    - espAttach no longer blocks; ChildAdded uses task.defer (zero-wait).
    - Items use DescendantAdded on workspace — already was event-driven, kept as is.
    - healthConns now stored per-object and cleaned up properly.
    - All helper tables use weak keys to avoid leaking destroyed instances.
]]
...
local esp = {
    ...
    playerConns = {},
    mapConns    = {},
    -- per-object connection tables (weak keys so GC can clean)
...
-- Forward-declare so espAttach and guard conn can reference each other
local espAttach
local espDetach
...
espAttach = function(obj, tag, color, isChar)
    ...
    -- Clean any stale connections first
    ...
    -- Find the root part without doing heavy searches
    local root
    if isChar then
        ...
        if not root then
            -- Only iterate children (not all descendants) for non-character objects
    ...
    -- Guard: if the highlight gets removed externally, re-attach
...
local function espDoItems(on)
    -- Only scan map content + ingame, NOT all workspace descendants
    local ig = getIngame()
...
local function espBindPlayers()
    ...
    if esp.killerFolder then
        table.insert(esp.playerConns, esp.killerFolder.ChildAdded:Connect(function(ch)
            -- task.defer instead of task.wait(0.2) — no thread blocking
            task.defer(function()
...
local espMapChildConns = {} -- connections scoped to the current map instance
...
local function espBindWorld()
    ...
    table.insert(esp.mapConns, ig.ChildAdded:Connect(function(obj)
        task.defer(function()
            ...
            if obj.Name == "Map" then
                task.wait(0.5) -- brief wait for map children to populate
    ...
    -- Item spawns — scoped to ingame DescendantAdded so we don't watch all of workspace
    ...
    -- Bind existing map content if already loaded
    ...
    -- Buildings already in scene
    if esp.buildings then espDoBuildings(true) end
end

-- Toggle callbacks — instant, no polling
secESP:Toggle({ Title="Killers",    Type="Checkbox", Default=esp.killers,    Callback=function(on) esp.killers=on;    cfg.set("espKillers",on);    espDoKillers(on)    end })
secESP:Toggle({ Title="Survivors",  Type="Checkbox", Default=esp.survivors,  Callback=function(on) esp.survivors=on;  cfg.set("espSurvivors",on);  espDoSurvivors(on)  end })
secESP:Toggle({ Title="Generators", Type="Checkbox", Default=esp.generators, Callback=function(on) esp.generators=on; cfg.set("espGenerators",on); espDoGenerators(on) end })
secESP:Toggle({ Title="Items",      Type="Checkbox", Default=esp.items,      Callback=function(on) esp.items=on;      cfg.set("espItems",on);      espDoItems(on)      end })
secESP:Toggle({ Title="Buildings",  Type="Checkbox", Default=esp.buildings,  Callback=function(on) esp.buildings=on;  cfg.set("espBuildings",on);  espDoBuildings(on)  end })
...
secMinion:Paragraph({
    Title   = "How it works",
    Content = "Tracks c00lkidd's pizza bots (orange), 1x1x1x1 zombies (green), and John Doe's shadow puddles (black disc with red glow). Each uses AncestryChanged to self-clean when the object is removed. Puddles also show a radius indicator that resizes as the puddle grows. Force Rescan clears and re-scans the whole workspace if anything was missed.",
})
...
-- Initial world setup — runs once, no repeat loops
task.spawn(function()
    task.wait(3)
...
lp.CharacterAdded:Connect(function()
    task.wait(4)
    -- Re-locate team folders (game may reload them on respawn)
    local pf = svc.WS:FindFirstChild("Players")
    if pf then
        esp.killerFolder   = pf:FindFirstChild("Killers")
        esp.survivorFolder = pf:FindFirstChild("Survivors")
...
secMinion:Button({ Title="🔄 Force Rescan", Callback=function() clearTag("pizza"); clearTag("zombie"); clearTag("puddle"); task.defer(function() scanPizza(); scanZombie(); scanPuddles() end) end })
...
secLMS:Paragraph({
    Title   = "How it works",
    Content = "Detects when Last Man Standing starts by counting alive survivors or checking the LMS sound. Swaps the game's LastSurvivor sound ID to your chosen track and plays it. Tracks are downloaded once and cached to your device so they load instantly on future rounds. Use Preload to download all tracks before a match so there's no delay.",
})
...
secLMS:Toggle({ Title="Auto-Play on LMS", Type="Checkbox", Flag = "musicOn", Default=music.on, Callback=function(on) music.on=on; cfg.set("musicOn",on); if on then music.thread=task.spawn(musicMonitor) else if music.thread then task.cancel(music.thread); music.thread=nil end; musicReset() end end })
...
local secSurvivors = tabSurSen:Section({ Title = "Survivors", Opened = true })

-- Elliot Aimbot
do
    local elliotSection = tabSurSen:Section({ Title = "Elliot Aimbot", Opened = false })
    ...
    elliotSection:Paragraph({
        Title   = "How it works",
        Content = "Hooks the UseActorAbility FireServer call to detect when you throw a pizza. While throwing, it aims your HRP and/or camera toward the lowest-health survivor. Show Pizza Arc draws a ballistic trajectory preview so you can see where the pizza will land. Require Throw Animation means it only aims during the actual throw animation, not all the time.",
    })
    elliotSection:Slider({ Title="Prediction Studs", Step=1, Value={Min=0,Max=50,Default=5}, Callback=function(v) elliotPredDist=v end })
    elliotSection:Slider({ Title="Aim Duration (s)", Step=0.1, Value={Min=0.1,Max=2,Default=0.5}, Callback=function(v) elliotThrowDur=v end })
    elliotSection:Slider({ Title="Pizza Throw Force", Step=5, Value={Min=50,Max=150,Default=80}, Callback=function(v) elliotThrowForce=v end })
    elliotSection:Slider({ Title="Arc Segments", Step=5, Value={Min=20,Max=100,Default=50}, Callback=function(v) elliotArcSegs=v end })
    elliotSection:Dropdown({ Title="Aimbot Type", Values={"HRP Aimbot","Camera Aimbot","Camera + Character"}, Value="Camera + Character", Callback=function(v) elliotAimType=v end })
    elliotSection:Toggle({ Title="Show Pizza Arc", Type="Checkbox", Default=false, Callback=function(v)
        elliotShowArc=v
    ...
    elliotSection:Toggle({ Title="Require Throw Animation", Type="Checkbox", Default=true, Callback=function(v) elliotRequireAnim=v end })
    elliotSection:Toggle({ Title="Enable Elliot Aimbot", Type="Checkbox", Default=false, Callback=function(v)
        elliotEnabled = v
        if v then
...
-- Chance Aimbot
do
    local chanceSection = tabSurSen:Section({ Title = "Chance Aimbot", Opened = false })
    ...
    chanceSection:Paragraph({
        Title   = "How it works",
        Content = "Aims your HRP toward the nearest killer using a BodyGyro. Velocity mode predicts where the killer will be based on their current speed. Ping mode accounts for your network latency. Hold-to-Aim lets you hold a key (default Q) to activate — release to stop. Anti-Bait caps killer velocity to their known max run speed so fake speed exploits don't throw off the prediction.",
    })
    chanceSection:Toggle({ Title="Enable Aimbot", Value=false, Callback=function(v) chanceAimEnabled=v end })
    chanceSection:Dropdown({ Title="Prediction Mode", Values={"Velocity","Ping","Look","LookPing"}, Value="Velocity", Callback=function(v) chancePredMode=v end })
    chanceSection:Input({ Title="Prediction Value", Value="0.5", Callback=function(v) local n=tonumber(v); if n then chancePredValue=n end end })
    chanceSection:Slider({ Title="Smooth Speed", Desc="Higher = snappier", Step=1, Value={Min=1,Max=30,Default=14}, Callback=function(v) chanceSmoothSpeed=v end })
    chanceSection:Toggle({ Title="Height-Aware Aim", Value=true, Callback=function(v) chanceHeightAim=v end })
    chanceSection:Dropdown({ Title="Aim Behavior", Values={"Normal","360"}, Value="Normal", Callback=function(v) chanceAimBehavior=v end })
    chanceSection:Input({ Title="Spin Duration", Value="0.5", Callback=function(v) local n=tonumber(v); if n then chanceSpinDur=n end end })
    chanceSection:Toggle({ Title="Anti Bait", Desc="Caps velocity to killer run speed", Value=true, Callback=function(v) chanceAntiBait=v end })
    chanceSection:Toggle({ Title="Hold-to-Aim", Value=true, Callback=function(v) chanceHoldToAim=v end })
    chanceSection:Dropdown({ Title="Aim Key", Values={"Q","E","R","T","F","G","X","C","V"}, Value="Q", Callback=function(v) chanceAimKey=Enum.KeyCode[v] end })
    chanceSection:Toggle({ Title="Message When Aim", Value=false, Callback=function(v) chanceMsgOnAim=v end })
    chanceSection:Input({ Title="Message Text", Callback=function(v) chanceMsgText=v end })
    chanceSection:Button({ Title="Ping Aim Preset", Callback=function()
        chanceAimEnabled=true; chancePredMode="Ping"; chancePredValue=1.5
        ui:Notify({ Title="Preset Applied", Content="Ping mode, value 1.5", Icon="check", Duration=3 })
    end })
    chanceSection:Button({ Title="Owner's Setting Opinion", Callback=function()
        ui:Notify({ Title="Owner's Opinion", Content="0.5 works best for 90-120 ping\nSame for ping 1.5\nIf high ping use 0.4 velocity\nElse 1.3 ping", Duration=10, Icon="info" })
    end })
    chanceSection:Button({ Title="Tester's Setting Opinion", Callback=function()
        ui:Notify({ Title="Tester's Opinion", Content="60 ping → 0.1-0.2\n120 ping → ~0.5\n200-300 ping → don't use", Duration=10, Icon="message-circle" })
    end })
...
do
    ...
    local function bsStartCrouchStab()
        ...
        if not btn then warn("[v1prware] CrouchStab: Crouch button not found"); return end
    ...
    bsSection:Paragraph({
        Title   = "How it works",
        Content = "Auto Backstab: detects when a killer is behind you within range and in the correct angle cone, then rotates your HRP to face them and optionally fires your dagger. Dash-Stab Tech: when you are crouching and a killer lunges at you, it snaps you to face their lunge direction so the dagger fires from the front for a counter-stab. Noclip disables collision with player parts only to make positioning easier.",
    })
    bsSection:Toggle({ Title="Enabled", Type="Checkbox", Default=false, Callback=function(v) bsEnabled=v end })
    bsSection:Toggle({ Title="Auto Use Dagger", Type="Checkbox", Default=false, Callback=function(v) bsDaggerEnabled=v end })
    bsSection:Toggle({ Title="Crouch Insta-Stab", Type="Checkbox", Default=false, Desc="Tap crouch → dagger fires after delay",
        Callback=function(v) bsCrouchStabEnabled=v; if v then bsStartCrouchStab() else bsStopCrouchStab() end end })
    bsSection:Slider({ Title="Crouch Stab Delay (s)", Step=0.01, Desc="Time after crouch before dagger fires.",
        Value={Min=0.00, Max=0.50, Default=0.10}, Callback=function(v) bsCrouchStabDelay=v end })
    ...
    bsSection:Slider({ Title="Aim Snap Delay (s)", Step=0.01, Value={Min=0.00,Max=0.40,Default=BS_AIM_SNAP_DELAY},
        Callback=function(v) BS_AIM_SNAP_DELAY=v end })
    bsSection:Slider({ Title="Flip Delay (s)", Step=0.01, Desc="SERVER_WINDUP 0.30s + ping.",
        Value={Min=0.28,Max=0.65,Default=BS_DASH_FLIP_DELAY}, Callback=function(v) BS_DASH_FLIP_DELAY=v end })
    bsSection:Slider({ Title="Lunge Hold (s)", Step=0.005, Desc="0.25 = uncrouched hitbox",
        Value={Min=0.10,Max=0.50,Default=BS_LUNGE_HOLD_DURATION}, Callback=function(v) BS_LUNGE_HOLD_DURATION=v end })
    bsSection:Slider({ Title="Depth Offset (st)", Step=0.25, Desc="Nudge forward into killer at flip.",
        Value={Min=0.0,Max=5.0,Default=BS_DEPTH_OFFSET}, Callback=function(v) BS_DEPTH_OFFSET=v end })
...
do
    ...
    secAutoTrick:Toggle({
        Title = "Auto Trick", Type = "Checkbox", Flag = "autoTrickOn", Default = false,
        Callback = function(on)
            atEnabled = on
            if on then
...
------------------------------------------------------------------------
-- SK8 Control (Veeronica Charge Override)
------------------------------------------------------------------------
...
do
    ...
    -- Whether to snap the mouse crosshair to the predicted target before each shot (PC only)
    ...
    -- Detect which executor mouse-move function is available
    local jd_mouseMoveRel = mousemoverel or mouse1move or (syn and syn.mouse_moverel) or nil

    -- Warp the desktop mouse to the predicted position.
    -- Locks MouseBehavior to Default (suppresses camera movement) during the warp,
    -- fires the crystal, then restores the original MouseBehavior on the next frame
    -- so the player gets full control back with zero fighting.
    local function jd_doMouseSnap(myHRP, killerHRP)
        if not jd_mouseMoveRel then return end

        -- Compute predicted world-space target
        local vel       = jd_getKillerVelocity(killerHRP)
        local predicted = killerHRP.Position + vel * jd_PREDICTION
        local target    = predicted + Vector3.new(0, jd_AIM_OFFSET, 0)
        ...
        -- Delta from current mouse to target
        local currentMouse = svc.Input:GetMouseLocation()
        local dx = screenPos.X - currentMouse.X
        local dy = screenPos.Y - currentMouse.Y

        -- Save current MouseBehavior so we can restore it exactly
        local prevBehavior = svc.Input.MouseBehavior

        -- Lock mouse: freeze camera input so moving the physical mouse during
        -- the warp doesn't rotate the camera and fight the snap
        pcall(function() svc.Input.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition end)

        -- Warp the desktop cursor to the predicted screen position
        pcall(jd_mouseMoveRel, dx, dy)

        -- Restore original MouseBehavior on the very next frame so the player
        -- gets control back cleanly with no jerk or fighting
        local restoreConn
    ...
    local function jd_applyPatch(actor)
        ...
        -- Set Device to Mobile so the server accepts crystal throws without a held button.
        -- This works on both PC and mobile — the server only checks the attribute value.
    ...
    -- Auto-fire loop: fires crystal repeatedly while enabled.
    -- On PC with snapMouse ON: snaps the camera to the ballistic angle just before
    -- FireServer so the throw registers at the predicted position, then unlocks the
    -- mouse so the player keeps free camera control between shots.
    task.spawn(function()
        while not jd_unloaded do
            ...
            if jd_aimbotOn and killerHRP then
                -- Wait two heartbeats so velocity data is fresh
                jd_Run.Heartbeat:Wait()
                jd_Run.Heartbeat:Wait()

                -- PC: warp desktop mouse to the predicted screen position, fire,
                -- then snap it back next frame so the player keeps control.
    ...
    jdMain:Toggle({Title="Enable Jane Doe Aimbot",Type="Checkbox",Default=false,Callback=function(on)
        jd_enabled=on; local actor=jd_getLocalActor()
        if on and not jd_patched and actor then jd_applyPatch(actor) end
    end})
    jdMain:Toggle({Title="Aimbot (Silent Aim)",Type="Checkbox",Default=false,Callback=function(on)
        jd_aimbotOn=on; if not on then jd_killerMotionData={} end
        local actor=jd_getLocalActor(); if on and not jd_patched and actor then jd_applyPatch(actor) end
    end})
    jdMain:Toggle({
        Title = "Snap Mouse to Prediction (PC)",
        Desc  = "Before each shot, snaps your crosshair to the predicted kill angle then frees the mouse.",
        Type  = "Checkbox", Default = false,
        Callback = function(on) jd_snapMouse = on end
    })
    jdMain:Divider()
    jdMain:Slider({Title="Aim Offset (Y)",Step=0.1,Value={Min=-5.0,Max=5.0,Default=jd_AIM_OFFSET},Callback=function(v) jd_AIM_OFFSET=v end})
    jdMain:Slider({Title="Prediction",Step=0.01,Value={Min=0.0,Max=1.0,Default=jd_PREDICTION},Callback=function(v) jd_PREDICTION=v end})
    jdMain:Slider({Title="Hold Duration (s)",Desc="Delay between crystal throws. Lower = faster.",Step=0.1,Value={Min=0.3,Max=2.0,Default=jd_HOLD_DURATION},Callback=function(v) jd_HOLD_DURATION=v end})
    ...
    jdAxe:Toggle({Title="Enable Axe Lock",Type="Checkbox",Default=false,Callback=function(on)
        jd_axeLockEnabled=on; if on then jd_axeStartDetection() else jd_axeStopDetection() end
    end})
